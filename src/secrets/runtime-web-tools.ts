...
function ensureLegacySerperWebSearchProvider(params: {
  ...
  if (params.providers.some((provider) => provider.id === "serper")) {
    return params.providers;
  }

  const search = isRecord(params.sourceConfig.tools?.web?.search)
    ? params.sourceConfig.tools?.web?.search
    : undefined;
  ...
  return [...params.providers, createLegacySerperWebSearchProviderEntry()];
}
...
async function resolveBundledWebSearchProviders(params: {
  ...
  if (onlyPluginIds && onlyPluginIds.length > 0) {
    ...
    if (bundled && bundled.length > 0) {
      return ensureLegacySerperWebSearchProvider({
        providers: bundled,
        env,
        sourceConfig: params.sourceConfig,
        configuredBundledPluginId: params.configuredBundledPluginId,
        onlyPluginIds,
      });
    }
