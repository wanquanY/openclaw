...
function isInternalWebchatContext(ctx: TemplateContext): boolean {
  const originatingChannel = normalizePromptMetadataString(ctx.OriginatingChannel)?.toLowerCase();
  const surface = normalizePromptMetadataString(ctx.Surface)?.toLowerCase();
  const provider = normalizePromptMetadataString(ctx.Provider)?.toLowerCase();
