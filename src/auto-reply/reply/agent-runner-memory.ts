...
import {
  buildCompressedSessionMemoryRecord,
...
async function appendPostCompactionRefreshPrompt(params: {
  ...
  const nextPrompts = refreshPrompts.filter((prompt) => !existingPrompt?.includes(prompt));
  if (nextPrompts.length === 0) {
    return;
  }

  params.followupRun.run.extraSystemPrompt = [existingPrompt, ...nextPrompts]
    .filter(Boolean)
    .join("\n\n");
}
...
export async function runPreflightCompactionIfNeeded(params: {
  ...
  const compressedSummary = result.result?.summary;
  if (compressedSummary && params.sessionStore && params.sessionKey) {
    const currentEntry = params.sessionStore[params.sessionKey] ?? entry;
    if (currentEntry) {
      const nextEntry: SessionEntry = {
        ...currentEntry,
        compressedSessionMemory: buildCompressedSessionMemoryRecord({
          summary: compressedSummary,
          updatedAt: Date.now(),
          compactionCount: currentEntry.compactionCount,
          tokensBefore: tokenCountForCompaction,
          tokensAfter: result.result?.tokensAfter,
        }),
      };
  ...
  await appendPostCompactionRefreshPrompt({
    ...
    sessionEntry: params.sessionStore?.[params.sessionKey] ?? entry,
    sessionStore: params.sessionStore,
    sessionKey: params.sessionKey,
  });
