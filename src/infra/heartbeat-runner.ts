...
export async function runHeartbeatOnce(opts: {
  ...
  if (!areHeartbeatsEnabled() && !forceExecEvent) {
...
export function startHeartbeatRunner(opts: {
  ...
  const run: HeartbeatWakeHandler = async (params) => {
    ...
    if (!areHeartbeatsEnabled() && !isExecEventReason) {
      return {
        status: "skipped",
        reason: "disabled",
      } satisfies HeartbeatRunResult;
      ...
      if (requestedSessionKey || requestedAgentId) {
        ...
        try {
          const res = await runOnce({
            cfg: state.cfg,
            agentId: runAgentId,
            heartbeat: targetAgent?.heartbeat ?? resolveHeartbeatConfig(state.cfg, runAgentId),
            reason,
