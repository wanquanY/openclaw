...
export async function startGatewayServer(
  ...
  const controlUiOriginsSeed = await maybeSeedControlUiAllowedOriginsAtStartup({
    config: cfgAtStart,
    writeConfig: writeConfigFile,
    log,
  });
  cfgAtStart = controlUiOriginsSeed.config;
  ...
  if (!minimalTestGateway && startupPluginIds.length > 0) {
    log.info(`gateway startup plugins: ${startupPluginIds.join(", ")}`);
  }
  ...
  try {
    ...
    const gatewayRequestContext: import("./server-methods/types.js").GatewayRequestContext = {
      ...
      eventsSubscribe: sessionEventSubscriptions.subscribe,
      eventsUnsubscribe: sessionEventSubscriptions.unsubscribe,
      eventsUnsubscribeAllForConn: sessionEventSubscriptions.removeConn,
      dedupe,
      wizardSessions,
      findRunningWizard,
      purgeWizardSession,
    ...
    logGatewayStartup({
      ...
      loadedPluginIds: pluginRegistry.plugins.map((plugin) => plugin.id),
      log,
      isNixMode,
    });
