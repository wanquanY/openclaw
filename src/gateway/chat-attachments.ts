      ...
      if (previewBase64) {
        try {
          ...
          const previewProvidedMime =
            typeof att.previewMimeType === "string"
              ? normalizeMime(att.previewMimeType)
              : undefined;
          ...
          if (previewSniffedMime && !isImageMime(previewSniffedMime)) {
            ...
            previewMimeType =
              previewSniffedMime ?? previewProvidedMime ?? sniffedMime ?? providedMime ?? finalMime;
          }
        } catch (err) {
          log?.warn(`attachment ${label}: preview invalid, dropping (${formatErrorMessage(err)})`);
        }
      }
