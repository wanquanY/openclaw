...
function dependencyVersionSatisfied(spec, installedVersion) {
  const normalizedSpec = typeof spec === "string" ? spec.trim() : "";
  if (!normalizedSpec) {
    return false;
  }
  // Non-semver specs such as git/file/workspace aliases still represent a
  // lockfile-pinned installed package. For runtime staging, the presence of
  // that resolved package in the root dependency tree is enough to reuse it.
  if (
    /^(?:file:|link:|workspace:|git\+|git:|github:|https?:|ssh:|npm:|patch:|portal:)/i.test(
      normalizedSpec,
    )
  ) {
    return true;
  }
  return semverSatisfies(installedVersion, normalizedSpec, { includePrerelease: false });
}
...
function resolveInstalledDependencyRoot(params) {
  ...
  for (const depRoot of candidates) {
    ...
    fallbackRoot ??= depRoot;
    if (dependencyVersionSatisfied(params.spec, installedVersion)) {
      return depRoot;
    }
  }
...
function resolveInstalledDirectDependencyNames(rootNodeModulesDir, dependencySpecs) {
  ...
  for (const [depName, spec] of Object.entries(dependencySpecs)) {
    ...
    // Mirror the actual installed runtime closure from the root dependency tree.
    // Hoisted installs can satisfy runtime resolution even when the package's
    // declared semver range does not exactly match the single hoisted version.
